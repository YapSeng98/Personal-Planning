  ...
  const generateInsight = useCallback(async (force = false) => {
    ...
    try {
      ...
      const system = lang === 'zh'
        ? '你是一位简洁、真诚的习惯教练。用2-3个短句：指出数据中一个真实的规律（比如哪天最容易中断），并给一个具体、可执行的建议。不要泛泛而谈，不要列表，最多一个表情。不要输出任何思考过程或前言，直接给出最终建议。用中文回复。'
        : "You are a concise, honest habit coach. In 2-3 short sentences: point out one real pattern in the data (e.g. which day breaks the chain), and give one specific, actionable suggestion. No generic advice, no lists, at most one emoji. Do not include any reasoning or preamble — output only the final insight."
